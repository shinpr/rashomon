...
|---------|-------------------------------|
| BP-001 Negative Instructions | Skill instructions with "don't" increase probability of the forbidden behavior. Convert to positive directives **except** in low-freedom contexts: safety-critical operations, exact command sequences, destructive actions, or order-dependent procedures where negative constraints ("Do not modify the command", "Do not add flags") are the most precise expression. |
